...
{
    /// <summary>
    /// 指数函数算法块（ EXP） Exponent
    /// </summary>
    [Serializable]
    public class PIDExp : PIDBindAlgorithm
    {
        /// <summary>
        /// 输入参数名
        /// </summary>
        ...
        protected override void InternalDoCalc()
        {
            ...
            double k = ConvertUtil.ConvertToDouble(calcParams[ParamK].Value, 1.0);
            double a = ConvertUtil.ConvertToDouble(calcParams[ParamA].Value, 1.0);
            if (ai == 0)
                this.calcResults[Result].Value = 1;
            else
                this.calcResults[Result].Value = k * System.Math.Pow(a, ai) + Convert.ToDouble(calcParams[ParamBias].Value);
