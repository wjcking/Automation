...
namespace Sinowyde.DOP.PIDAlgorithm.Special
{
    ...
    /// </summary>
    [Serializable]
    public class PIDMonthOnline : PIDBindAlgorithm
    {
        #region 变量
        ...
        /// 一个月里多少个小时
        /// </summary>
        private double HoursInMonth(DateTime dateTime)
        {
        ...
        {
            this.calcResults[ResultAO] = new PIDAlgorithmVar(ResultAO, 0, PIDVarInputType.Init, PIDVarDataType.AM); //初应该到数据库里取？
        }
        ...
        protected override void InternalDoCalc()
        {
            ...
            {
                if (!cTime.Month.Equals(LastCalcTime.Month))//处理跨度问题 
                {
                    var cZeroTime = new DateTime(cTime.Year, cTime.Month, cTime.Day, 0, 0, 0);//当天0点
                    var tmBefore = (cZeroTime - LastCalcTime).TotalMilliseconds;//零点之前段
                    var tmAfter = (cTime - cZeroTime).TotalMilliseconds;//零点之后段
                ...
                if (Math.Abs(calcResults[ResultAO].Value - HoursInMonth(cTime)) <= (5000 / hourMilliseconds))
                    calcResults[ResultAO].Value = HoursInMonth(cTime);

                calcResults[ResultAO].Value = MathEx.InRange(calcResults[ResultAO].Value, 0, HoursInMonth(cTime)); //处理值域问题
            }
        }
